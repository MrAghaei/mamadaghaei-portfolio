        ...
        <section id="hire-me-form-section">
          ...
          <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label htmlFor="email" className="sr-only">{t('common.emailAddress')}</label>
                <input
                  type="email"
                  name="email"
                  id="email"
            ...
            <div>
              <label htmlFor="message" className="sr-only">{t('common.message')}</label>
              <textarea
                name="message"
                id="message"
                rows={5}
            ...
            {status === 'success' && <p className="text-sm text-accent-green mt-4">{t('common.formSuccess')}</p>}
            {status === 'error' && errors.form && <p className="text-sm text-accent-red mt-4">{errors.form}</p>}
             {status === 'error' && !errors.form && (Object.keys(errors).length > 0 && errors.constructor === Object && Object.values(errors).some(err => err !== undefined && typeof err === 'string')) && <p className="text-sm text-accent-red mt-4">{t('common.formFixErrors')}</p>}
          </form>
        </section>
      </div>
    </div>
